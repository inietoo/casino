...
<head>
    ...
    <style>
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .bingo-cell.line-win.marked::after { background: rgba(33,150,243,0.65) !important; border-color: #2196f3 !important; }
...
<body>
    ...
    <script>
        ...
        // CORRECCIÓN: primera carga no muestra overlays (evita que reaparezcan al recargar)
        ...
        function updateBingo() {
            ...
            .then(state => {
                ...
                const phaseLabels = { waiting: 'Fase de Compra', playing: 'Sorteo en curso', finished: 'Ronda Finalizada' };
                ...
                if (drawn.length > 0) {
                    ...
                    if (drawn.length !== knownDrawnCount) {
                        let histHtml = '';
                        drawn.slice(Math.max(0, drawn.length - 15), drawn.length - 1).reverse()
                             .forEach(b => histHtml += `<div class="mini-ball">${b}</div>`);
                ...
                for (const uid in players) {
                    ...
                    pList += `<div class="player-item"><span>${p.avatar} ${p.username}</span><span style="color:#f8c8dc;">${count} 🎫</span></div>`;
                }
                ...
                // CORRECCIÓN: primera carga — sincronizar estado sin mostrar overlays
                if (isFirstLoad) {
                    lastLinePaid     = !!state.line_paid;
                    lastBingoWinners = JSON.stringify(state.bingo_winners || null);
                    isFirstLoad      = false;
                    return;
                }

                // Anuncio LÍNEA (solo cuando cambia false → true)
        ...
        // CORRECCIÓN: confirmación antes de salir durante el sorteo
        function leaveRoom() {
            const phaseLabel = document.getElementById('phase-label').innerText;
            if (phaseLabel === 'Sorteo en curso') {
                if (!confirm('⚠️ ¿Seguro que quieres salir? Si abandonas durante el sorteo perderás tus cartones y no recibirás ningún premio.')) {
                    return;
                }
            }

...
function dealerPlay(array &$state): void {
    foreach ($state['dealer_cards'] as &$c) {
        if ($c === 'hidden') $c = array_shift($state['_hidden']);
    }
    unset($state['_hidden']);
...
function settleHands(PDO $pdo, int $room_id, array &$state): void {
    ...
    foreach ($state['players'] as $uid_key => &$player) {
        ...
        updateBjStats($pdo, $room_id, $real_uid, $player, $bet, $payout, $result, $state['dealer_cards']);
    }
    unset($player);
...
function updateBjStats(PDO $pdo, int $room_id, int $uid, array $player, float $bet, float $payout, string $result, array $dealerCards): void {
    try {
        ...
        $pdo->prepare("
            INSERT INTO blackjack_hand_log
        ...
        // CORRECCIÓN: antes el catch estaba vacío; ahora al menos logueamos
        error_log('BJ stats update error (uid=' . $uid . '): ' . $e->getMessage());
    }
}
...
if ($action === 'bet') {
    ...
    // CORRECCIÓN: toda la operación dentro de una transacción con FOR UPDATE real
    try {
        ...
        if ($state['phase'] !== 'betting') {
            $pdo->rollBack();
            echo json_encode(['error' => 'No es fase de apuestas']); exit;
        }

        $uid = (string)$user_id;
        if (!isset($state['players'][$uid])) {
            $pdo->rollBack();
            echo json_encode(['error' => 'No estás en esta sala']); exit;
        }
        ...
        if ($amount < 10 || $amount > 500 || $amount > $balance) {
            $pdo->rollBack();
            echo json_encode(['error' => 'Apuesta inválida o saldo insuficiente']); exit;
        }
        ...
        error_log('BJ bet error: ' . $e->getMessage());
        echo json_encode(['error' => 'Error al registrar la apuesta']);
    }
    exit;
}
...
if ($action === 'hit') {
    try {
        ...
        if ($state['phase'] !== 'playing' || $base_turn_uid !== (string)$user_id) {
            $pdo->rollBack();
        ...
        if ($total > 21) {
            $state['players'][$uid]['status'] = 'bust'; nextTurn($state);
        } elseif ($total === 21) {
            $state['players'][$uid]['status'] = 'standing'; nextTurn($state);
        }
        ...
        if ($pdo->inTransaction()) $pdo->rollBack();
        error_log('BJ hit: ' . $e->getMessage());
        echo json_encode(['error' => 'Error al pedir carta']);
    }
    exit;
}
...
if ($action === 'stand') {
    try {
        ...
        if ($pdo->inTransaction()) $pdo->rollBack();
        error_log('BJ stand: ' . $e->getMessage());
        echo json_encode(['error' => 'Error al plantarse']);
    }
    exit;
}
...
// ─── DOUBLE ──────────────────────────────────────────────────────────────────
if ($action === 'double') {
    // CORRECCIÓN: transacción completa con FOR UPDATE real
    try {
        $pdo->beginTransaction();
        $state = getState($pdo, $room_id);
        ...
        if ($balance < $bet) {
            $pdo->rollBack();
            echo json_encode(['error' => 'Saldo insuficiente para doblar']); exit;
        }
        ...
        if ($pdo->inTransaction()) $pdo->rollBack();
        error_log('BJ double: ' . $e->getMessage());
        echo json_encode(['error' => 'Error al doblar']);
    }
    exit;
}
...
if ($action === 'split') {
    ...
    try {
        ...
        if (strpos($uid, '_split') !== false) {
            $pdo->rollBack();
            echo json_encode(['error' => 'No puedes dividir más de una vez']); exit;
        }

        if (count($state['players'][$uid]['cards']) !== 2) {
            $pdo->rollBack();
            echo json_encode(['error' => 'Solo puedes dividir con 2 cartas']); exit;
        }
        ...
        if (cardValue($c1) !== cardValue($c2)) {
            $pdo->rollBack();
            echo json_encode(['error' => 'Las cartas no tienen el mismo valor']); exit;
        }
        ...
        if ($balance < $bet) {
            $pdo->rollBack();
            echo json_encode(['error' => 'Saldo insuficiente para dividir']); exit;
        }
        ...
        if ($pdo->inTransaction()) $pdo->rollBack();
        error_log('BJ split: ' . $e->getMessage());
        echo json_encode(['error' => 'Error al dividir']);
    }
    exit;
}
...
if ($action === 'reset_table') {
    ...
    $stmt->execute([$room_id]);
    if ($stmt->fetchColumn() != $user_id) {
        echo json_encode(['error' => 'Solo el líder puede reiniciar la mesa']); exit;
    }
